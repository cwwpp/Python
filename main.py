...
def draw_arc(window, center, radius, start_angle, end_angle):
    p1 = center + (radius+50) * np.array([math.cos(start_angle),math.sin(start_angle)])
    p2 = center + (radius+50) * np.array([math.cos(end_angle),math.sin(end_angle)])
...
ORANGE = (255,165,0)
RED = (255,0,0)
CIRCLE_CENTER = np.array( [WIDTH/2,HEIGHT/2], dtype = np.float64 ) # data type = float
CIRCLE_RADIUS = 150
BALL_RADIUS = 5
GRAVITY = 0.2
